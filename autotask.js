...
var apipass = require("./conf").apipass;
var self;
  ...
  self.soap(s,function(e,r,b){
    ...
    for(var i in rrr){
      if(rrr[i].email == rname || rname.indexOf(rrr[i].username + "@") == 0){
        console.log(rname + " logged in (" + rrr[i].id + ")");
...
atask.prototype.getEntityInfo.tickets = function(d,callback){
  var s = soap.model.tickets();
  var fields = [];
...
atask.prototype.getEntityInfo.tasks = function(d,callback){
  ...
  self.soap(s,function(e,r,b){
    var rrr = soap.parse(b.toString());
    self.getEntityInfo.tickets(d,function(data){
      for(var j in data){rrr.push(data[j]);}
      callback(rrr);
    });
  });
};
